...
static bool insideTriangleMy(int x, int y, const Vector4f* _v) {
    Vector3f v[3];
    for (int i = 0; i < 3; i++)
        v[i] = { _v[i].x(),_v[i].y(), 1.0 };
    Vector3f p(x,y,1.0);
    ...
    Vector3f p1=  pa.cross(pb);
    Vector3f p2 = pb.cross(pc);
    Vector3f p3 = pc.cross(pa);
    bool a1= p1.dot(p2) >= 0 && p2.dot(p3) >= 0 && p3.dot(p1)>=0;
    bool a2 = p1.dot(p2) < 0 && p2.dot(p3) < 0 && p3.dot(p1)< 0;
    if(a1||a2)
        return true;
    return false;
}
    ...
    for (int y = BMin[1]; y < BMax[1]; ++y)
    {
        for (int x = BMin[0]; x < BMax[0]; ++x)
        {
            if(insideTriangle(x, y, t.v))
            {
               ...
               float z_interpolated = (a * v[0].z() + b * v[1].z() + c * v[2].z()) / (a + b + c);
               ...
               // printf("%f,,", a + b + c);
                ...
                if (z_interpolated < depth_buf[ind])
                {
                    ...
                    // // auto interpolated
                    Vector3f interpolated_color = (a * t.color[0] + b * t.color[1] + c * t.color[2])/(a+b+c);
